...
describe("Get User controller", () => {
    ...
    test("Deveria retornar 404 se o usuario nao for encontrado.", async () => {
        ...
        const res = await request(app).get("/user/123").send()
    ...
    test("Deveria retornar 200 se um usuario for listado.", async () => {
        ...
        expect(res.body).toHaveProperty("message", "Usuario listado com sucesso")
    })
    ...
    test("Deveria retornar 500 se o repository virar exceção", async () => {
        ...
        const result = await request(app).get(`/user/123`).send();
