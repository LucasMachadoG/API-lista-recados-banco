...
describe("Get Recado controller", () => {
    beforeAll(async () => {
    ...
    test("Deveria retornar 404 se o recado nao for encontrado.", async () => {
        ...
        const res = await request(app).get(`/user/${User.id}/recados/123`).set('Authorization', token).send()
    ...
    test("Deveria retornar 200 se o recado for listado com sucesso.", async () => {
        ...
        const res = await request(app).get(`/user/${User.id}/recados/${Recado.id}`).set('Authorization', token).send().expect(200)

        expect(res).toBeDefined();
        expect(res).toBeTruthy();
        expect(res.body.message).toEqual("Recado listado com sucesso");
    })
    ...
    test("Deveria retornar 500 se o repository virar exceção", async () => {
        ...
        jest.spyOn(getRecadosUsecase.prototype, "execute").mockImplementation(() => {
            throw new Error("Erro simulado");
          });
      
        const result = await request(app).get(`/user/123/recados/124`).set('Authorization', token).send();
